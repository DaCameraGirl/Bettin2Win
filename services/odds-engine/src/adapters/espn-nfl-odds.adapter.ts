...
/**
 * No-key ESPN NFL scoreboard with DraftKings moneyline prices when ESPN exposes
 * them. Uses the rolling scoreboard window (no single-day date filter).
 */
export class EspnNflOddsAdapter implements SportAdapter {
  readonly sport = "football";
  readonly provider = "espn-nfl-odds";
  ...
  async fetchEvents(): Promise<AdapterResult> {
    try {
      ...
      if (!res.ok) {
        return { mode: "live", events: [], message: `ESPN NFL scoreboard ${res.status}` };
      }

      const events = normalizeEspnNflOddsScoreboard(await res.json());
      ...
      return {
        mode: "live",
        events,
        message: `${priced}/${events.length} NFL games with DraftKings moneyline odds from ESPN`,
      };
    } catch (err) {
      return {
        mode: "live",
        events: [],
        message: err instanceof Error ? err.message : "ESPN NFL fetch failed",
      };
    }
  }
}

export function normalizeEspnNflOddsScoreboard(body: unknown): SportEvent[] {
  return array(field(object(body), "events"))
    .map(normalizeEspnNflOddsEvent)
    .filter((event): event is SportEvent => event !== null);
}

export function normalizeEspnNflOddsEvent(raw: unknown): SportEvent | null {
...
function runner(
  ...
  const runnerId = `espn-nfl-odds:${eventId}:${side}`;
